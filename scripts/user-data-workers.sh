...
sudo docker-compose -f docker-compose.workers.yml up --build -d --scale video-processor=${WORKER_REPLICAS}
...
WORKER_COUNT=$(sudo docker ps --filter "name=video-processor" --format "{{.Names}}" | wc -l)
...
sudo chown ec2-user:ec2-user /home/ec2-user/workers-info.txt
